    ...
    <section class="section dashboard">
                            ...
                            <form method="GET" class="mb-0" action="{{ route('dashboard') }}">
                                <div class="input-group input-group-sm">
                                    <select name="type" class="form-select select-g" onchange="this.form.submit()" aria-label="Filter by type">
                                        ...
                                        <option value="lease" {{ ($type ?? '') == 'lease' ? 'selected' : '' }}>Leases</option>
                                    </select>
                                    @if(request('period'))
        ...
        @if(isset($role) && $role === 'property_lister')
            ...
            @else
                ...
                <div class="row">
                    <div class="col-lg-4 col-md-6 col-sm-6 mb-4"> /* Active Listings */ <div class="card h-100 shadow border-start border-primary border-4"> <div class="card-body d-flex align-items-center"> <div class="flex-shrink-0 me-3"><i class="bi bi-building-check text-primary fs-2"></i></div> <div class="flex-grow-1"> <div class="text-muted text-uppercase small fw-bold">My Active Listings</div> <div class="fs-4 fw-bold">{{ $activeListingsCount ?? 0 }}</div> <a href="{{ route('lister.properties.index', ['status' => 'approved']) }}" class="small text-decoration-none d-block mt-1">View Active</a> </div> </div> </div> </div>
                    <div class="col-lg-4 col-md-6 col-sm-6 mb-4"> /* Pending Submissions */ <div class="card h-100 shadow border-start border-warning border-4"> <div class="card-body d-flex align-items-center"> <div class="flex-shrink-0 me-3"><i class="bi bi-hourglass-split text-warning fs-2"></i></div> <div class="flex-grow-1"> <div class="text-muted text-uppercase small fw-bold">Pending Submissions</div> <div class="fs-4 fw-bold">{{ $pendingListingsCount ?? 0 }}</div> <a href="{{ route('lister.properties.index', ['status' => 'pending']) }}" class="small text-decoration-none d-block mt-1">Review Pending</a> </div> </div> </div> </div>
                    <div class="col-lg-4 col-md-6 col-sm-6 mb-4"> /* Total Earnings */ <div class="card h-100 shadow border-start border-success border-4"> <div class="card-body d-flex align-items-center"> <div class="flex-shrink-0 me-3"><i class="bi bi-cash-coin text-success fs-2"></i></div> <div class="flex-grow-1"> <div class="text-muted text-uppercase small fw-bold">Total Earnings <span class="text-lowercase">({{ ($period ?? 'all') }})</span></div> <div class="fs-4 fw-bold">${{ number_format($totalEarnings ?? 0, 2) }}</div> <small class="text-muted">From completed transactions</small> </div> </div> </div> </div>
                </div>
                <div class="row mt-2">
