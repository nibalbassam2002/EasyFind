<div class="row g-3">
    ...
    <div class="col-md-6">
        ...
        <select class="form-select @error('category_id') is-invalid @enderror" id="property_category_id"
            name="category_id" required>
            <option value="" disabled {{ !old('category_id', $property->category_id ?? '') ? 'selected' : '' }}>
                Select Type...</option>
            @isset($categories)
    ...
    <div class="row g-3">
        <div id="sub-category-wrapper" class="col-md-6" style="display: none;"> {{-- إضافة id وإخفاء --}}
            <label for="sub_category_id" class="form-label required">Sub Category (Commercial Type)</label>
            {{-- تعديل الـ label للتوضيح --}}
            <select class="form-select @error('sub_category_id') is-invalid @enderror" id="sub_category_id"
                name="sub_category_id"> {{-- إزالة required مؤقتاً أو جعله شرطياً --}}
                ...
                @isset($subCategories)
                    ...
                    @foreach ($subCategories->where('parent_id', $commercialCategoryId) as $subCategory)
                        <option value="{{ $subCategory->id }}" data-parent-id="{{ $subCategory->parent_id }}"
                            {{ old('sub_category_id', $property->sub_category_id ?? '') == $subCategory->id ? 'selected' : '' }}>
                            {{ $subCategory->name }}
                        </option>
                    @endforeach
                @endisset
    ...
    <div class="col-md-4">
        ...
        <input type="number" step="1" {{-- أو 100 أو أي قيمة منطقية لخطوة السعر --}}
            ...
            value="{{ old('price', $property->price ?? '') }}" required min="50" {{-- حد أدنى منطقي للسعر (عدّله حسب عملتك) --}}
            ...
            @isset($currencies)
                @foreach ($currencies as $currency)
                    <option value="{{ $currency }}"
                        {{ old('currency', $property->currency ?? 'ILS') == $currency ? 'selected' : '' }}>
                        {{ $currency }}</option>
                @endforeach
            @endisset
        </select>
    ...
    <div class="col-md-6">
        ...
        <select class="form-select @error('governorate_id') is-invalid @enderror" id="governorate_id"
            name="governorate_id" required>
            <option value="" disabled selected>Select Governorate...</option>
            @isset($governorates)
                @foreach ($governorates as $governorate)
                    <option value="{{ $governorate->id }}"
                        {{ old('governorate_id', $property->listarea->governorate_id ?? '') == $governorate->id ? 'selected' : '' }}
    ...
    <div class="col-md-3">
        ...
        <input type="number" class="form-control @error('area') is-invalid @enderror" id="area"
            name="area" value="{{ old('area', $property->area ?? '') }}" required min="1"
            placeholder="Total/Built Area">
        @error('area')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    <div class="col-md-3">
        ...
        <input type="number" class="form-control @error('land_area') is-invalid @enderror" id="land_area"
            name="land_area" value="{{ old('land_area', $property->land_area ?? '') }}" min="0"
            placeholder="(If House/Villa/Land)">
        @error('land_area')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    <div class="col-md-3">
        ...
        <select class="form-select @error('property_condition') is-invalid @enderror" id="property_condition"
            name="property_condition">
            <option value=""
                {{ old('property_condition', $property->property_condition ?? '') == '' ? 'selected' : '' }}>Select...
            </option>
            <option value="new"
                {{ old('property_condition', $property->property_condition ?? '') == 'new' ? 'selected' : '' }}>New
            </option>
            <option value="used"
                {{ old('property_condition', $property->property_condition ?? '') == 'used' ? 'selected' : '' }}>Used
            </option>
            <option value="needs_renovation"
                {{ old('property_condition', $property->property_condition ?? '') == 'needs_renovation' ? 'selected' : '' }}>
                Needs Renovation</option>
        </select>
    ...
    <div class="col-md-3">
        ...
        <select class="form-select @error('finishing_type') is-invalid @enderror" id="finishing_type"
            name="finishing_type">
            <option value=""
                {{ old('finishing_type', $property->finishing_type ?? '') == '' ? 'selected' : '' }}>Select...</option>
            <option value="full"
                {{ old('finishing_type', $property->finishing_type ?? '') == 'full' ? 'selected' : '' }}>Full</option>
            <option value="semi"
                {{ old('finishing_type', $property->finishing_type ?? '') == 'semi' ? 'selected' : '' }}>Semi-Finished
            </option>
            <option value="none"
                {{ old('finishing_type', $property->finishing_type ?? '') == 'none' ? 'selected' : '' }}>Unfinished
            </option>
        </select>
    ...
    <div class="col-12">
        <div id="specific-fields-container">
            ...
            <div id="residential-fields" class="row g-3 category-specific-fields" style="display: none;">
                <hr class="my-3">
                <h6 class="text-muted">Residential Details</h6>
                <div class="col-md-3">
                    <label for="rooms" class="form-label">Rooms</label>
                    <input type="number" class="form-control @error('rooms') is-invalid @enderror" id="rooms"
                        name="rooms" value="{{ old('rooms', $property->rooms ?? '') }}" min="0">
                    @error('rooms')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" class="form-control @error('floors') is-invalid @enderror" id="floors"
                        name="floors" value="{{ old('floors', $property->floors ?? '') }}" min="0">
                    @error('floors')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-3" id="apartment-floor-number" style="display: none;"> {{-- خاص بالشقق --}}
                    ...
                    <input type="number" class="form-control" id="apartment_floor_num" name="apartment_floor_num"
                        value="{{ old('apartment_floor_num', $property->apartment_floor_num ?? '') }}"
                        min="0">
                </div>
                <div class="col-md-6">
                    <label for="view_type" class="form-label">View</label>
                    <input type="text" class="form-control @error('view_type') is-invalid @enderror"
                        id="view_type_residential" name="view_type"
                        value="{{ old('view_type', $property->view_type ?? '') }}"
                        placeholder="e.g., Sea, Garden, Main Street">
                    @error('view_type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div id="land-fields" class="row g-3 category-specific-fields" style="display: none;">
                <hr class="my-3">
                <h6 class="text-muted">Land Details</h6>
                <div class="col-md-4">
                    <label for="land_type" class="form-label">Land Specific Type</label>
                    <input type="text" class="form-control @error('land_type') is-invalid @enderror"
                        id="land_type" name="land_type" value="{{ old('land_type', $property->land_type ?? '') }}"
                        placeholder="e.g., Agricultural, Residential Plot">
                    @error('land_type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div id="commercial-fields" class="row g-3 category-specific-fields" style="display: none;">
                <hr class="my-3">
                <h6 class="text-muted">Commercial Details</h6>
                <div class="col-md-4">
                    <label for="commercial_type" class="form-label">Commercial Type</label>
                    <select class="form-select @error('commercial_type') is-invalid @enderror" id="commercial_type"
                        name="commercial_type">
                        <option value=""
                            {{ old('commercial_type', $property->commercial_type ?? '') == '' ? 'selected' : '' }}>
                            Select type...</option>
                        <option value="office"
                ...
                <div class="col-md-8">
                    ...
                    <input type="text" class="form-control @error('commercial_purpose') is-invalid @enderror"
                        id="commercial_purpose" name="commercial_purpose"
                        value="{{ old('commercial_purpose', $property->commercial_purpose ?? '') }}"
                        placeholder="e.g., Restaurant, Store, Clinic, Storage">
                    @error('commercial_purpose')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            </div>
            ...
            <div id="tent-fields" class="row g-3 category-specific-fields" style="display: none;">
                <hr class="my-3">
                <h6 class="text-muted">Tent Details</h6>
                <div class="col-md-4">
                    <label for="tent_type" class="form-label">Tent Type</label>
                    <input type="text" class="form-control @error('tent_type') is-invalid @enderror"
                        id="tent_type" name="tent_type" value="{{ old('tent_type', $property->tent_type ?? '') }}"
                        placeholder="e.g., Family Tent, Relief Tent">
                    @error('tent_type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div id="caravan-fields" class="row g-3 category-specific-fields" style="display: none;">
                <hr class="my-3">
                <h6 class="text-muted">Caravan Details</h6>
                <div class="col-md-4">
                    <label for="caravan_type" class="form-label">Caravan Type</label>
                    <input type="text" class="form-control @error('caravan_type') is-invalid @enderror"
                        id="caravan_type" name="caravan_type"
                        value="{{ old('caravan_type', $property->caravan_type ?? '') }}"
                        placeholder="e.g., Static, Touring">
                    @error('caravan_type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
    ...
    <div class="col-md-12">
        ...
        <textarea class="form-control @error('additional_details') is-invalid @enderror" id="additional_details"
            name="additional_details" rows="3"
            placeholder="e.g., Land is flat, contains olive trees, close to main services...">{{ old('additional_details', $property->additional_details ?? '') }}</textarea>
    ...
    <div class="col-md-12">
        ...
        <input type="text" class="form-control @error('location') is-invalid @enderror" id="location"
            ...
            placeholder="e.g., 31.9539, 35.9106">
                        ...
                        @foreach ($currentImages as $imagePath)
                            <div class="col position-relative">
                                @if (Storage::disk('public')->exists($imagePath))
                                    <img src="{{ Storage::url($imagePath) }}" class="img-thumbnail" alt="Property Image"
                                    ...
                                    <div class="form-check position-absolute top-0 end-0 m-1 bg-light rounded p-1">
                                        ...
                                        <label class="form-check-label small" for="delete_img_{{ $loop->index }}"
    ...
    <div class="col-md-12">
        ...
        <input type="url" class="form-control @error('video_url') is-invalid @enderror" id="video_url"
            name="video_url" value="{{ old('video_url', $property->video_url ?? '') }}" placeholder="https://...">
        @error('video_url')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
