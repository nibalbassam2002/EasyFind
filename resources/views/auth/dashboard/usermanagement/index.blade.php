                                ...
                                <td class="text-center table-actions-cell">
                                    <div class="d-inline-flex align-items-center">
                                        ...
                                        <a href="{{ route('admin.users.edit', $user->id) }}"
                                            class="btn btn-sm btn-outline-gold me-1 px-2" title="Edit User">
                                            <i class="bi bi-pencil-fill"></i>
                                        </a>
                                        @if (in_array($user->role, ['admin', 'content_moderator']) && $user->id !== Auth::id())
                                            <form action="{{ route('chat.initiate.direct') }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                <input type="hidden" name="recipient_id" value="{{ $user->id }}">
                                                <button type="submit" class="btn btn-sm btn-outline-primary me-1 px-2"
                                        ...
                                        @if (Auth::id() !== $user->id)
                                            <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST"
                                                class="d-inline delete-form"> {{-- تم إزالة ms-1 من هنا لأن زر العرض أصبح قبله --}}
                                                @csrf
                                                @method('DELETE')
                                                {{-- ▼▼▼ تعديل: تغيير لون زر الحذف ▼▼▼ --}}
                                                <button type="submit" class="btn btn-sm btn-outline-danger px-2"
                                                    title="Delete User"> {{-- تغيير btn-outline-dark1 إلى btn-outline-danger --}}
