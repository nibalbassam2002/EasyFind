...
@section('content')
                    ...
                    <div class="card-body p-2 p-md-3">
                        <div class="row text-center g-2">
                            <div class="col-6 col-md-3">
                                <a href="{{ route('lister.properties.create', ['purpose' => 'sale']) }}"
                                    class="btn btn-outline-gold w-100 py-2">
                                    <span class="small">Add for Sale</span>
                                    <i class="bi bi-plus-circle-dotted d-block fs-4 mb-1"></i>
                                </a>
                            </div>
                            <div class="col-6 col-md-3">
                                <a href="{{ route('lister.properties.index', ['purpose' => 'sale']) }}"
                            ...
                            <div class="col-6 col-md-3">
                                <a href="{{ route('lister.properties.create', ['purpose' => 'rent']) }}"
                                    class="btn btn-outline-gold w-100 py-2">
                            ...
                            <div class="col-6 col-md-3">
                                <a href="{{ route('lister.properties.index', ['purpose' => 'rent']) }}"
                                    class="btn btn-outline-secondary w-100 py-2">
    ...
    <div class="px-4 py-5 my-5 text-center">
        ...
        <div class="col-lg-6 mx-auto">
            <p class="lead mb-4">Whether you're in search of a new residence, an investment opportunity, or simply
                exploring
        ...
        <div class="row text-center justify-content-center g-4">
            ...
            @foreach ($categories as $category)
                ...
                @php
                    ...
                    if ($category->slug == 'houses') {
                        $iconClass = 'bi bi-house-door';
                    }
                    if ($category->slug == 'apartments') {
                        $iconClass = 'bi bi-building';
                    }
                    if ($category->slug == 'caravans') {
                        $iconClass = 'bi bi-truck';
                    }
                    if ($category->slug == 'tents') {
                ...
                <div class="col-6 col-md-3 col-lg-auto"> {{-- استخدم lg-auto لمرونة أفضل --}}
                    <a href="{{ route('frontend.properties', ['category_slug' => $category->slug]) }}"
                        ...
                        <div class="filter-item p-3 {{ request('category_slug') == $category->slug ? 'active' : '' }}">
                            <i class="{{ $iconClass }}"
                                style="font-size: 2rem; min-height: 32px; display: inline-block; line-height: 1;"></i>
                            ...
                            <div class="card property-card h-100 shadow-sm">
                                @php
                                    ...
                                    if (is_string($property->images)) {
                                        // احتياطي إذا لم يتم تحويلها بعد
                                ...
                                @auth {{-- أظهر أيقونة المفضلة فقط للمستخدمين المسجلين --}}
                                    <div class="favorite-icon" data-property-id="{{ $property->id }}">
                                        ...
                                        <i
                                            class="bi {{ $property->is_favorited ?? false ? 'bi-heart-fill text-danger' : 'bi-heart' }}"></i>
                                    </div>
                                @endauth
                                ...
                                <div class="card-body d-flex flex-column"> {{-- d-flex و flex-column لمحاذاة العنوان للأسفل --}}
                                    ...
                                    <p class="fw-semibold mb-2 property-title-clamp text-dark"
                                    ...
                                    <div class="d-flex text-muted property-info mb-2 small">
                                        ...
                                        @if ($property->bathrooms)
                                            <div class="me-3"><i class="bi bi-droplet"></i> {{ $property->bathrooms }}
                                                bath</div>
                                        @endif
                                        @if ($property->area)
                                            {{-- تأكد أن اسم الحقل صحيح (مثلاً area أو size) --}}
                                            <div class="me-3"><i
            ...
            <div class="bg-dark text-white rounded-4 p-4 p-md-5">
                <div class="text-center mb-4">
                    ...
                    <p class="text-white-50">
                        We value your feedback. Whether it's a complaint, improvement suggestion, or any other idea, please
                        share it with us.
                    </p>
                </div>
                ...
                <form method="POST" action="{{ route('feedback.store') }}">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label for="feedback_type" class="form-label fw-semibold">Feedback Type<span
                                    class="text-danger">*</span></label>
                            <select class="form-select @error('feedback_type') is-invalid @enderror" id="feedback_type"
                                name="feedback_type" required>
                                <option value="" disabled {{ old('feedback_type') ? '' : 'selected' }}>Choose the note
                                    type...</option>
                                <option value="complaint" {{ old('feedback_type') == 'complaint' ? 'selected' : '' }}>
                                    Complaint</option>
                                <option value="suggestion" {{ old('feedback_type') == 'suggestion' ? 'selected' : '' }}>
                                    Suggestion</option>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('feedback_subject') is-invalid @enderror"
                                id="feedback_subject" name="feedback_subject" value="{{ old('feedback_subject') }}"
                                placeholder="Example: Property display issue">
                            @error('feedback_subject')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-12">
                            <label for="feedback_message" class="form-label fw-semibold">Your Message<span
                                    class="text-danger">*</span></label>
                            <textarea class="form-control @error('feedback_message') is-invalid @enderror" id="feedback_message"
                                name="feedback_message" rows="5" placeholder="Please write your feedback in detail..." required>{{ old('feedback_message') }}</textarea>
    ...
    @else
        ...
        <div class="container my-5" id="feedback-section-guest">
            <div class="bg-dark text-white rounded-4 p-4 p-md-5 text-center">
                <h4 class="fw-bold">Share your feedback with us</h4>
                <p class="text-white-50">
                    Your feedback matters to us! Let us know your thoughts <a
                        href="{{ route('login') }}?redirect={{ url()->current() }}#feedback-section"
                        class="text-warning fw-bold">log in </a> or <a href="{{ route('register') }}"
